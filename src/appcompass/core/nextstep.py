...
def decide_next_step(
    ...
    if state.evidence_count == 0:
        return NextStep(
            ...
            how=(
                "대상자 5명에게 물어봅니다. 30분이면 충분합니다.",
                "'근거' 화면에서 '예시로 양식 채우기'를 눌러 형식을 확인합니다.",
                "예시 내용을 실제로 들은 것으로 바꿔 등록합니다.",
                "등록 뒤 '분석 실행'을 다시 누르면 판단이 확정됩니다.",
            ),
            screen="evidence",
            button_text="근거 화면으로 가기",
        )
    ...
    if confidence < policy.hold_threshold:
        ...
        return NextStep(
            ...
            how=(
                ...
                "근거 하나가 여러 항목을 지지하도록 '지지 항목'을 넉넉히 고르세요.",
                "등록 뒤 '분석 실행'을 다시 누릅니다.",
            ),
            screen="evidence",
            button_text="근거 화면으로 가기",
        )
