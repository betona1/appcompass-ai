...
| 탭 | 목적 | 완료 조건 |
|---|---|---|
| **① 다음 할 일** | **지금 할 일 하나를 알려주고 그 화면으로 이동** | — |
...
진단만 하고 끝나면 쓸모가 없습니다. 결과를 **바로 개발에 넘길 수 있는 형태**로 내보냅니다.

```text
아이디어 → 구조화 → 진단 → MVP 확정 → TECHSPEC.md → 구현
                                     └→ 엑셀 백로그 → 담당자 배정
                                                        ↓
                          IMPROVEMENT.md ← 구현 상태 + 행동 데이터
```

**막막하면 `① 다음 할 일` 탭만 보면 됩니다.** 지금 할 일 하나를 알려주고 그 화면으로 데려다줍니다.

**기술 명세 (TECHSPEC.md)** 에는 기능마다 ID와 명세 블록이 붙습니다.
...
> **`[결정 필요]`가 이 문서의 핵심입니다.**
> 진단 결과에서 도출할 수 없는 항목은 지어내지 않습니다.
> 사실처럼 적힌 빈 명세는 빈칸보다 위험하기 때문입니다.

**작업용 엑셀**은 시트가 목적별로 나뉘어 있습니다 —
요약 / 평가 점수 / 경고 / 할 일 / 타깃 후보 / MVP 백로그 / 측정 이벤트 / 근거.
담당·기한·완료 기준 칸은 사람이 채우도록 비어 있습니다.
...
> 자세한 내용은 [NOTICE.md](NOTICE.md) 참고.
...
참고 서적의 내용과 사용된 오픈소스 라이브러리의 라이선스는 [NOTICE.md](NOTICE.md)에 정리했습니다.
